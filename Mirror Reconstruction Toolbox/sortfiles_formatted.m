...
if num_numeric_parts ~= 1
    ...
    for i = 1 : num_numeric_parts
        ...
        % Print the first 6 numbers in that sequence (if sequence length greater than 6, else print numel sequence).
        fprintf(['%d: ', ...
            repmat('%d, ', 1, min(6 - 1, numel(unique_numeric_parts)) - 1), ...
            '%d\n'], ...
            i, location_wise_parts{i} ...
        );
    end

    numeric_idx = input('Select the number corresponding to the part you want to use for sorting: ');
