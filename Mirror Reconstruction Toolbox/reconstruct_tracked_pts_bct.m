...
viewnames = default.VIEW_NAMES_LONG(view_labels);
...
for j = 1 : num_views
    figH(j) = figure('Name', sprintf('%s: Pixel Reprojections', viewnames(j)), 'Units', 'Normalized', 'Position', figpos_reprojection{j});
end
...
for j = 1 : num_views
    ...
    title(sprintf('%s: Pixel Reprojections for Estimated World Coordinates\nFrame %d/%d', ...
        viewnames(j), frame_num, num_frames) ...
    );
