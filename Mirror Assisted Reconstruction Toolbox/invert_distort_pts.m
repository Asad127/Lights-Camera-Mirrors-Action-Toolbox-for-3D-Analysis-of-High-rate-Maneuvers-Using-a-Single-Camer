function undistorted_pts = invert_distort_pts(points, dist_coefs, intrinsics, max_iterations, tolerance)
%{
Invert an array of 2D distorted points to recover the original undistorted points
using an iterative fixed-point algorithm. This function computes undistorted points
such that applying the distortion transform (with the same distortion coefficients
and intrinsics) to the output undistorted points will recover the input distorted points.

Effectively computes the inverse map of distort_pts.m.

The main idea is to use the given distorted points as the initial guess for the
undistorted points, then iteratively refine the guess by applying the forward
distortion to the estimated undistorted points and comparing the result to the
original distorted points.

INPUTS
===============================================================================
points:
    2xN matrix of [x; y] DISTORTED pixel coordinates. Can also be Nx2.
dist_coefs:
...
===============================================================================
undistorted_pts:
    Matrix of undistorted [x; y] pixel coordinates. Shape matches input.
%}
...
for iter = 1 : max_iterations
    ...
    % Compute the distorted coordinates from the current undistorted estimate
    % (if this was a distorted estimate, we would have gotten the undistorted
    % estimate instead).
    ...
    % Compute error between the target distorted coordinates and the current
    % distorted estimate.
...
% Map the converged undistorted normalized coordinates back to pixel coordinates.
U_undistorted = Xc_current_undistorted_norm * fx + cx;
V_undistorted = Yc_current_undistorted_norm * fy + cy;

% Output as 2xN matrix.
undistorted_pts = [U_undistorted; V_undistorted];

% Ensure output format matches input format regarding transpose.
