...
if ~frames_dir
    ...
    if ~isfolder(frames_dir)
        fprintf( ...
            ...
            'Please run\n`setup_project_structure.m` and ensure that the relevant ' ...
            'directory is created.\n'] ...
        )
        mkdir(frames_dir)  
    end
...
while true
    ...
    frame_extension = input( ...
        ['[PROMPT] Enter image extension for extracted calibration frames (blank = default image ' ...
        'extension): '], 's' ...
    );
...
vid_to_frames(calib_vid_filepath, frames_dir, default.VID_FRAMENAME_FMT, frame_extension, start_time_seconds, stop_time_seconds);
fprintf('Done extracting frames. Frames stored at:\n\t%s\n', frames_dir)

fprintf('\n...launching "calib_select_img_subset.m" to get a subset of calibration images from video frames...\n')
