function [framewise_tracked_pixels_converted, num_frames, valid_frames, ...
    invalid_frames, num_pts] = reconstruction_process_trackfile(trackfile_csv, num_views)
%{
...
nan_rows = all(isnan(framewise_tracked_pixels_dltdv8a), 2);
valid_frames = find(~nan_rows);
invalid_frames = find(nan_rows);
framewise_tracked_pixels_dltdv8a = framewise_tracked_pixels_dltdv8a(~nan_rows, :);
